...
echo -e "Atualizando os arquivos de configuração do Apache2 e do PHP, aguarde..."
	...
	cp -v conf/lamp/awstats.higinoserver.pti.intra.conf /etc/awstats/ &>> $LOG
...
sleep 5
#
echo -e "Editando o arquivo de configuração awstats.higinoserver.pti.intra.conf, pressione <Enter> para continuar."
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /etc/awstats/awstats.higinoserver.pti.intra.conf
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
