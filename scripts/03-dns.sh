...
# Windows Powershell.: 
#	nslookup pti.intra
#	nslookup 192.168.0.rev
#	nslookup higinoserver
#	nslookup higinoserver.pti.intra
#	ipconfig /displaydns
#	ping pti.intra
...
echo -e "Atualizando os arquivos de configuração do Bind DNS Server, aguarde..."
	...
	cp -v conf/dns/{pti.intra.hosts,192.168.0.rev} /var/lib/bind/ &>> $LOG
	...
	chown -v root:bind /var/lib/bind/{pti.intra.hosts,192.168.0.rev} &>> $LOG
...
sleep 5
#
echo -e "Editando o arquivo de configuração 192.168.0.rev, pressione <Enter> para continuar."
	# opção do comando: &>> (redirecionar a saída padrão)
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /var/lib/bind/192.168.0.rev
	named-checkzone $DOMAINREV /var/lib/bind/192.168.0.rev &>> $LOG
	named-checkzone $NETWORK /var/lib/bind/192.168.0.rev &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
